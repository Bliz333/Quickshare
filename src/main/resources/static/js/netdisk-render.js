...
function isImageFile(file) {
    ...
    if (['jpg', 'jpeg', 'png', 'gif', 'bmp', 'svg', 'webp'].includes(ext)) return true;
    return false;
}
...
async function previewFile(index) {
    ...
    if (previewDecision.kind === 'image') {
        ...
        <div class="relative inline-block">
            ...
            <button onclick="event.stopPropagation(); downloadFile(${index})"
                class="absolute bottom-4 left-4 z-50 bg-black/50 hover:bg-black/70 text-white text-xs px-3 py-1.5 rounded-full backdrop-blur-md transition-all border border-white/10 flex items-center gap-2">
        ...
        const originalUrl = getOriginalUrl(file);
        try {
            const res = await BrowserSession.request(originalUrl);
