...
- 位于 `mobile/` 的 Expo / React Native 移动端客户端，以及生成后的 Android / iOS 原生工程
...
- 仓库中还包含一个位于 `mobile/` 的已落库移动端客户端，以及生成后的原生 Android / iOS 工程 `mobile/android/` 与 `mobile/ios/`。

## 移动端客户端

仓库中已经存在移动端代码：

- Expo / React Native 入口：`mobile/App.tsx`
- 移动端源码目录：`mobile/src/`
- 原生 Android 工程：`mobile/android/`
- 原生 iOS 工程：`mobile/ios/`
...
- 文件浏览、上传、预览、下载与分享创建
- 取件查询与转存网盘
- 同账号 transfer sync 与 incoming/outgoing 任务展示
- Android debug/release 构建、模拟器启动与支付 deeplink 验证
- iOS readiness 检查与 CI simulator 构建/安装/启动验证
