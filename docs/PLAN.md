...
- 结果：
  ...
  - 当前阶段已完成；若后续继续扩面，可再补真实公网商户回跳与更多登录后网盘 CRUD 回归
...
- 后续若继续优先扩面：
  - 登录后网盘 CRUD 页面级回归可继续拆成更小的稳定用例，而不是单条综合流程
- 条件允许时可继续补浏览器自动化，而不是只依赖手工点击回归。
- 当前 Playwright + Chromium 基线已经可用；未来如需继续扩大页面级覆盖面，可优先补真实公网商户回跳场景与更多登录后网盘 CRUD 交互。
